...
Déjalo corriendo, para probar que funciona pon la siguiente url en el navegador: [http://localhost:3000/api/clients](http://localhost:3000/api/clients)
...
> Fíjate que aquí para _$q_ no hemos definido una variable miembro como _$http_ directamente le hemos plantado el _private_ en el constructor, esto es un azúcar de TypeScript que nos permite definir una variable miembro y asignarle el valor de un parámetro del constructor.
...
- Vámonos ahora a nuestra página de clientes y creamos un _controller_ y traernos el servicio de _clienteAPIService_ lo asignamos a la página (vamos a usar el azúcar de TS para definir la variable miembro y asignarle el valor del parámetro del constructor).
...
- Queremos cargar los datos justo cuando se carga la página, para ellos vamos a usar _$OnInit_ (como vimos en ejemplos anteriores este método se ejecuta justo después del constructor cuando ya está montando el componente en el dom):
...
- Ahora ese parámetro lo vamos a recibir en el componente hijo como binding de entrada:
...
Fíjate que lo qué le pasamos por los parámetros lo evalúa cómo un enlace a una variable miembro de los bindings o del controlador, si quisiéramos pasar un valor literal deberíamos añadir unas comillas simples adicionales (como estaba antes con la card harcodeada).
